...
.then(data =>{ 
    globalHome = data
    renderTemplate (craftCards(globalHome.events), homejs)
...
//Funcion para filtrar categorias
    ...
    function checkFilter (touchs, categoriesList){
        ...
        let filters = categoriesList.filter(eventoValue => values.includes(eventoValue.category.toLowerCase()))  //el includes es generico
        if (values.length === 0){
    ...
    function filtroCruzado(evento){
        ...
        const filterPerFind = searchBar (search, globalHome.events)
